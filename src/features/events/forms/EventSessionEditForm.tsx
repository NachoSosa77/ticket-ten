...
import { updateEventSessionAction } from "@/app/actions/event-session.actions";
...
type EventSessionEditFormProps = {
  eventId: number;
  session: {
    id: number;
    venueName: string;
...
export default function EventSessionEditForm({
  eventId,
  session,
}: EventSessionEditFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      const result = await updateEventSessionAction(eventId, session.id, fd);

      if (!result.success) {
        setErrors(result.errors ?? {});
        setMessage(result.message ?? "No se pudo actualizar la función.");
      }
    });
  };
  ...
  return (
    <div className="max-w-2xl space-y-4 rounded-2xl border p-6">
      <div>
        <h1 className="text-2xl font-semibold">Editar función</h1>
        <p className="text-sm text-gray-500">
          Modifica fecha, hora y lugar de la función.
        </p>
      </div>

      <div className="text-sm text-gray-600">
        Estado actual: <span className="font-medium">{session.status}</span>
      </div>
